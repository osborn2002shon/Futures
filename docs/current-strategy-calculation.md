...
flowchart TD
    ...
    J -- "無新穿越" --> Z
    J -- "有新穿越" --> K{"是否與目前有效建議同一觸發"}
    K -- "是" --> Z
    K -- "否" --> L["尋找歷史同向事件並計算價格"]
    L --> M{"新事件是否為 waiting_entry 且仍有有效建議"}
    M -- "是" --> N["舊建議以目前價格結束為停利/停損"]
    M -- "否" --> Q["新增事件與狀態歷程"]
    N --> O{"現在是否仍有有效建議"}
    O -- "是" --> P["新事件記為 suppressed_active_recommendation"]
    O -- "否" --> Q
    P --> Q
    Q --> R["再次推進新建議生命週期"]
    R --> S["只將新增或狀態改變寫入 TXT"]
...
5. 若新觸發已是目前有效建議的同一觸發，不重複新增。
6. 若新觸發能建立 `waiting_entry` 且仍有不同的有效建議，先以目前價格把舊建議結束為 `take_profit` 或 `stop_loss`。
7. 若結束後仍有有效建議，新事件才記為 `suppressed_active_recommendation`；否則建立新建議。
8. 新建議建立後，再立即用目前已保存的 tick 推進一次狀態。
...
- 若與目前有效建議是同一根 15 分 K 的同一方向觸發：
  - 視為已處理過的同一觸發，不重複新增。
- 若新觸發可建立 `waiting_entry` 且目前已有不同有效建議：
  - 不論舊建議是 `waiting_entry` 或 `entered`，都先用新觸發當下價格結束舊建議。
  - 舊建議會依該價格相對於原建議方向，轉為 `take_profit` 或 `stop_loss`，並釋放 `IsActive`。
  - 新事件接著嘗試建立為新的有效建議。
- 若轉換期間資料庫仍存在其他有效建議：
  - 新觸發只保存稽核事件，狀態為 `suppressed_active_recommendation`。
  - 不建立第二筆有效建議。
- 若新觸發無法建立有效價格：
  - 照常保存為非 active 的 `no_reference_event`、`invalid_price_structure` 或 `insufficient_price_data`。
...
| `cancelled` | 否 | 舊版保留狀態：入場前發生反方向 SMA76 穿越 |
| `suppressed_active_recommendation` | 否 | 轉換期間仍有有效建議，新事件只留紀錄 |
...
7. 驗證新有效觸發會先把既有 `waiting_entry` 或 `entered` 結束為 `take_profit` / `stop_loss`，再建立新建議。
8. 驗證同一觸發棒不重複新增。
